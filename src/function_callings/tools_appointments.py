...
class AppointmentTools:
    ...
    def _parse_and_normalize_datetime(self, text: str) -> Optional[str]:
        ...
        try:
            ...
            default_dt = now.replace(month=now.month, day=now.day, hour=9, minute=0, second=0, microsecond=0)
            dt = isoparse(text) if "T" in text or ":" in text else parse(text, default=default_dt)
    ...
    @function_tool(name="prepare_appointment_details", description="Validate and normalize name, email, and requested date/time make sure capture the date and time as digits not as words. Returns normalized payload and a friendly confirmation message.")
    async def prepare_appointment_details(
        ...
        try:
            ...
            dt_obj = datetime.fromisoformat(payload["appointment_datetime"].replace("Z", "+00:00"))
            dt_friendly = dt_obj.strftime("%A, %b %d at %I:%M %p")
            # Remove leading zero from hour and lowercase am/pm for natural speech
            dt_friendly = dt_friendly.replace(" at 0", " at ", 1).replace("AM", "am").replace("PM", "pm")
        except Exception:
    ...
    @function_tool(name="confirm_and_send_appointment", description="Send a previously prepared appointment payload (name, email, appointment_datetime) to the configured webhook with retries.")
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    async with resp:
                        if 200 <= resp.status < 300:
                            logger.info("appointment webhook succeeded", extra={"status": resp.status})
                            return {
                                "status": "ok",
    ...
    @function_tool(name="schedule_appointment", description="Single-step scheduling that validates and sends to the webhook. Prefer the two-step flow for confirmations.")
    async def schedule_appointment(
        ...
        if not webhook_url:
            return (
                "Configuration error: APPOINTMENT_WEBHOOK_URL is not set. Please set it and try again."
            )

        if not _validate_email(email):
            return (
                "The email provided seems invalid. Please provide a valid email address."
            )
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    async with resp:
                        if 200 <= resp.status < 300:
                            logger.info("appointment webhook succeeded", extra={"status": resp.status})
                            return (
                                "Your appointment details have been submitted. We will confirm shortly."
                            )
                        text = await resp.text()
                        logger.warning(
                            "appointment webhook failed",
                        ...
                        if 500 <= resp.status < 600 and attempt < max_retries:
                            ...
                            continue
                        return (
                            f"Failed to submit appointment (status {resp.status}). Please try again later."
                        )
            except Exception as exc:
                logger.exception("appointment webhook exception", exc_info=exc)
                if attempt < max_retries:
                    ...
                    continue
                return (
                    "There was a network error submitting your appointment. Please try again later."
                )
