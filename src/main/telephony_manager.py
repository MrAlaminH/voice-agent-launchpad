...
class TelephonyManager:
    ...
    async def handle_inbound_call(
        ...
        call_id = (
            call_id
            or f"inbound_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{phone_number}"
        )
        ...
        call_metadata = CallMetadata(
            ...
            metadata={"caller_id": caller_id, "source": "twilio_sip"},
        )
    ...
    async def make_outbound_call(
        ...
        call_id = (
            call_id
            or f"outbound_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{phone_number}"
        )
        ...
        call_metadata = CallMetadata(
            ...
            room_name=room_name,
            start_time=datetime.now(),
            metadata={"agent_instructions": agent_instructions, "source": "twilio_sip"},
        )
    ...
    async def end_call(self, call_id: str) -> bool:
        ...
        try:
            ...
            logger.exception(
                "Failed to end call", extra={"call_id": call_id, "error": str(exc)}
            )
            return False

    async def update_call_status(
        self, call_id: str, status: CallStatus, **kwargs
    ) -> bool:
        """
    ...
    async def add_call_transcript(
        self, call_id: str, transcript_entry: dict[str, Any]
    ) -> bool:
        """
        ...
        try:
            response = await self.lkapi.room.create_sip_participant(
                api.CreateSIPParticipantRequest(
                    room=room_name, sip_trunk_id=trunk_id, phone_number=phone_number
                )
            )
    ...
    async def _send_call_webhook(
        self, call_metadata: CallMetadata, event_type: str
    ) -> None:
        """
        ...
        try:
            payload = {
                ...
                "start_time": call_metadata.start_time.isoformat()
                if call_metadata.start_time
                else None,
                "end_time": call_metadata.end_time.isoformat()
                if call_metadata.end_time
                else None,
                "duration_seconds": call_metadata.duration_seconds,
                "sip_participant_id": call_metadata.sip_participant_id,
